...
// ---- Tag → consultant mapping ----
export async function mapTag(
  odooTagId: number,
  odooTagName: string,
  ...
  const supabase = await createClient();
  if (!consultantId) {
    await supabase.from("odoo_tag_map").delete().eq("odoo_tag_id", odooTagId);
  } else {
    await supabase.from("odoo_tag_map").upsert(
      {
...
export async function runOdooSync() {
  ...
  // Lookups: tag → consultant, country name → id, existing lead ids.
  const [{ data: tagRows }, { data: countryRows }, { data: existing }] =
    await Promise.all([
      admin.from("odoo_tag_map").select("odoo_tag_id, consultant_id"),
  ...
  const tagToConsultant = new Map<number, string>();
  (tagRows ?? []).forEach((t) => {
    if (t.consultant_id) tagToConsultant.set(Number(t.odoo_tag_id), t.consultant_id);
  });
  ...
  for (const lead of leads) {
    ...
    // Owner via the first mapped tag on the lead.
    const consultantId = (lead.tag_ids || [])
      .map((id) => tagToConsultant.get(id))
      .find(Boolean);
    if (!consultantId) {
      skippedUnmapped++;
      continue;
    ...
    const countryName = Array.isArray(lead.country_id)
      ? lead.country_id[1]
      : null;
    ...
    newRows.push({
      ...
      applicant_name:
        str(lead.contact_name) || str(lead.partner_name) || str(lead.name) || "Unnamed",
  ...
  const summary = `Imported ${created} new · ${skippedExisting} already imported · ${skippedUnmapped} skipped (no consultant tag) · ${leads.length} cards in stage`;
  await admin
    .from("odoo_config")
    .update({
